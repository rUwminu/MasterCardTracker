...
namespace MasterCardTracker
{
    public partial class Form1 : Form
    {
        ...
        private async void saveScanHis(string WOid, string MSid)
        {
            // Delay This task for last task to complete sql read and connection to close *Testing Beta
            await Task.Delay(TimeSpan.FromSeconds(5));

            string insertinfo = "INSERT INTO demo.masterc_record(mcr_no, mcr_wo_no, mcr_location, mcr_datetime) VALUES (@mcr_no, @mcr_wo_no, @mcr_location, @mcr_datetime)";
            ...
            upcmd.Parameters.AddWithValue("@mcr_location", initialstate);
                        ...
                        while (reader.Read())
                        {
                            ...
                            if (initialstate == "Final")
                            {
                                ...
                                else if (lastlocation != initialstate)
                                {
                                    ...
                                    saveScanHis(woid, msid);

                                    return;
                                }
                            }
                            else if (initialstate == "Extrusion" && (proccessStep == "FS" || proccessStep == "S1O" || proccessStep == "S12O" || proccessStep == "S13O"))
                            {
                            ...
                            else if (initialstate == "Printing" && (proccessStep == "FS" || proccessStep == "S2O" || proccessStep == "S12O" || proccessStep == "S23O"))
                            {
                                if (msid != "" && lastlocation == initialstate)
                                {
                                    label3.Text = "MasterCard Already Here.";
                            ...
                            else if (initialstate == "Cutting" && (proccessStep == "FS" || proccessStep == "S3O" || proccessStep == "S23O" || proccessStep == "S13O"))
                            {
                                if (msid != "" && lastlocation == initialstate)
                                {
                                    label3.Text = "MasterCard Already Here.";
        ...
        private void textBox1_TextChanged(object sender, EventArgs e)
        {
            if (textBox1.Text != "")
            {
                string woallsql = "SELECT demo.masterc_record.mcr_no, demo.masterc_record.mcr_location, demo.workorder.id, demo.workorder.WO_master, demo.workorder.WO_proccess " +
                                  "FROM demo.workorder " +
        ...
        {
            string woallsql2 = "SELECT demo.workorder.id AS wono, demo.workorder.WO_master AS womsno, demo.masterc_record.mcr_location AS mslocation, demo.masterc_record.mcr_datetime AS msdate " +
                                  "FROM demo.workorder " +
                                  "LEFT JOIN demo.masterc_record ON demo.workorder.WO_master = demo.masterc_record.mcr_no " +
                                  "WHERE demo.workorder.id = '" + textBox1.Text + "' " +
                                  "ORDER BY demo.masterc_record.id DESC";
